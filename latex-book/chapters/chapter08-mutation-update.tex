...
Mutation is the process of changing the value of one or more genes in a genome~\cite{goldberg1989genetic, mitchell1996introduction, back1996evolutionary}. More specifically, it involves:
...
In Holland's original GA~\cite{holland1975adaptation, goldberg1989genetic}:
...
A population of size $N$ chromosomes in one generation is replaced by $N$ new individuals in the next generation~\cite{de1975analysis, whitley1994genetic}. However, to preserve the best solutions:
...
In the steady-state model~\cite{whitley1994genetic, smith1998replacement}:
...
\textbf{Important Note:} There are no definitive rules for setting GA parameters~\cite{wolpert1997no, grefenstette1986optimization}. Parameter selection relies on:
